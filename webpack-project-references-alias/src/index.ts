...
type TsConfig = { compilerOptions?: { outDir?: string; rootDir?: string}, references?: { path: string}[]};

type PackageJson = { main?: string, name: string }

export function getAliasForProject(project: string | undefined): { [key: string]: string } {
	if (typeof project === 'undefined') {
		project = process.cwd()
	}

	const rootProjectPath = resolveTsConfig(project);
	const alias = flattenObject(getReferencedProjectsRecursive(rootProjectPath).map(getAliasFor));
	return alias;
}
...
function getReferencedProjectsRecursive(tsConfigPath: string): string[] {
	...
	return [tsConfigPath, ...dedupe(flatten(directReferencedProjects.map(getReferencedProjectsRecursive)))]
...
function getReferencedProjects(tsConfigPath: string) {
	...
	const references = config.references?.map(o => o.path).map(p => path.join(projectDir, p)).map(resolveTsConfig);
												return references ?? [];
}
...
function findPackageJsonDir(dir: string): string | undefined {
	return keepApplying(dir, path.dirname).takeUntilStable().first(isPackageDir);
}

function tryGetPackageInfo(dir: string): {name: string, main?: string, packageDir: string } | undefined {
	try {
		const packageDir = findPackageJsonDir(dir);
		const packageJson = require(path.join(packageDir, "package.json")) as PackageJson;
...
function getAliasFor(tsConfigPath: string) {
	...
	const outDir = hasExplicitOutDir ? path.join(projectRootDir, config.compilerOptions.outDir) : rootDir

	const packageInfo = tryGetPackageInfo(projectRootDir);
	const alias = {};
	if (typeof packageInfo !== 'undefined') {
		const name = packageInfo.name;
		const main = path.join(packageInfo.packageDir, packageInfo.main || './index.js');
