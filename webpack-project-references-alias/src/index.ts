...
function tryGetPackageInfo(dir: string): {name: string, main?: string } | undefined {
	try {
...
function getAliasFor(tsConfigPath: string) {
	...
	const rootDir = path.join(packageRootDir, config.compilerOptions.rootDir || "");
	const outDir = hasExplicitOutDir ? path.join(packageRootDir, config.compilerOptions.outDir) : rootDir
